...
export class ModelLoader {
            ...
            const model = await new Promise((resolve, reject) => {
                this.loader.load(
                    ...
                    (xhr) => {
                        ...
                        console.log(`Cargando modelo ${modelPath}: ${Math.round(percent)}%`);
                    },
                    // onError callback
                    (error) => {
    ...
    async loadCharacter(characterType) {
        ...
        if (!config) {
            throw new Error(`Configuración no encontrada para el personaje: ${characterType}`);
        }
            ...
            if (config.animations) {
                for (const [name, animPath] of Object.entries(config.animations)) {
                    try {
                        ...
                        console.warn(`No se pudo cargar la animación ${name} para ${characterType}:`, error);
                    }
                }
            }
